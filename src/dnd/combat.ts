...
export class Combat {
    constructor(
      ...
      public defender: Creature,
      public attackerAttackBonus: number,
      public attackerDamageRoll: DamageRoll
    ) {}
    ...
    fight(): {
      ...
      const attackerRoll = Math.floor(Math.random() * 20) + 1 + this.attackerAttackBonus;
      const attackSucceeded = attackerRoll >= this.defender.armorClass;
      let damageDealt = 0;
      if (attackSucceeded) {
        for (let i = 0; i < this.attackerDamageRoll.numDice; i++) {
          damageDealt += Math.floor(Math.random() * this.attackerDamageRoll.numSides) + 1;
        }
      }
